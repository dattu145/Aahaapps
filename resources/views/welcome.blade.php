        ...
        <div class="fixed inset-y-0 right-0 z-[100] w-full max-w-xs bg-white shadow-2xl transform transition-transform"
             ...
             <div class="h-full flex flex-col overflow-y-scroll py-6 bg-white shadow-xl">
                ...
                <div class="mt-6 relative flex-1 px-4 sm:px-6">
                    ...
                    <div class="mt-8 px-4 flex flex-col items-center space-y-6">
                        <!-- Social Icons -->
                        <div class="flex items-center justify-center gap-6">
                            @if(isset($globalSettings['social_email']) && $globalSettings['social_email'])
                                <a href="mailto:{{ $globalSettings['social_email'] }}" class="w-10 h-10 rounded-full bg-gray-50 text-gray-600 hover:bg-indigo-50 hover:text-indigo-600 flex items-center justify-center transition-all duration-300 shadow-sm border border-gray-100">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"></path></svg>
                                </a>
                            @endif

                            @if(isset($globalSettings['social_linkedin']) && $globalSettings['social_linkedin'])
                                <a href="{{ $globalSettings['social_linkedin'] }}" target="_blank" class="w-10 h-10 rounded-full bg-gray-50 text-gray-600 hover:bg-indigo-50 hover:text-indigo-600 flex items-center justify-center transition-all duration-300 shadow-sm border border-gray-100">
                                    <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z"/></svg>
                                </a>
                            @endif

                            @if(isset($globalSettings['social_web']) && $globalSettings['social_web'])
                                <a href="{{ $globalSettings['social_web'] }}" target="_blank" class="w-10 h-10 rounded-full bg-gray-50 text-gray-600 hover:bg-indigo-50 hover:text-indigo-600 flex items-center justify-center transition-all duration-300 shadow-sm border border-gray-100">
                        ...
                        <!-- Company Address -->
                        @if(isset($globalSettings['company_address']) && $globalSettings['company_address'])
                            <div class="text-center px-4">
                                <p class="text-xs text-gray-500 leading-relaxed max-w-[200px] mx-auto" style="font-size: {{ $globalSettings['company_address_size'] ?? '12' }}px;">
